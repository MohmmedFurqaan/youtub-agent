...
const TimelineDiagram: React.FC<{ data: TimelineData; sceneStartMs?: number; sceneDurationMs?: number }> = ({ data, sceneStartMs = 0, sceneDurationMs = 5000 }) => {
  ...
  const revealAt = enterEvent ? Math.max(0, (enterEvent.atMs / 1000 * fps) - sceneStartMs / 1000 * fps) : 0;
  const progress = interpolate(frame, [revealAt, revealAt + 50], [0, 1], { extrapolateRight: "clamp", easing: Easing.bezier(0.16, 1, 0.3, 1) });
