...
const CompareCard: React.FC<{
  ...
  accent: string;
  delayFrames: number;
}> = ({ x, y, width, height, label, accent, delayFrames }) => {
...
const ComparisonDiagram: React.FC<BaseDiagramProps> = ({ data, event, sceneStartMs = 0, sceneDurationMs }) => {
  ...
  // Derive left/right labels from event or nodes
  const leftLabel = data?.left
    || event?.left
    || nodes[0]?.label
    || "Left";
  const rightLabel = data?.right
    || event?.right
    || nodes[1]?.label
    || "Right";
  ...
  return (
    <svg width="100%" height="100%" viewBox="0 0 1080 1920" preserveAspectRatio="xMidYMid meet">
      ...
      <CompareCard x={160} y={620} width={300} height={360} label={leftLabel} accent="#60a5fa" delayFrames={0} />
      <CompareCard x={620} y={620} width={300} height={360} label={rightLabel} accent="#34d399" delayFrames={10} />
