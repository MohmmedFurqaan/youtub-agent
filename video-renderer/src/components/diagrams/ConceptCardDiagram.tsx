...
const ConceptCardDiagram: React.FC<BaseDiagramProps> = ({ data, event, sceneStartMs = 0, sceneDurationMs }) => {
  ...
  // Derive title and subtitle from data or event context
  const title = data?.title
    || (event?.left && event?.right
      ? `${event.left} vs ${event.right}`
      : event?.result
        ? event.result
        : nodes[0]?.label
          ? `${nodes[0].label} Explained`
          : "Concept")
    || nodes[0]?.label
    || "Concept";
  ...
  const revealEvent = timeline.find((e) => e.type === "enter" || e.type === "reveal-node");
  const revealAt = revealEvent
    ? Math.max(0, Math.min(revealEvent.atMs / 1000 * fps, frame))
    : 0;
  const barProgress = interpolate(revealAt, [0, 60], [0.2, 1], { extrapolateRight: "clamp", easing: Easing.bezier(0.16, 1, 0.3, 1) });
  ...
  if (nodes.length === 0 && !title) {
    return (
      <svg width="100%" height="100%" viewBox="0 0 1080 1920" preserveAspectRatio="xMidYMid meet">
        ...
        <text x={540} y={960} fill="#94a3b8" fontSize={28} fontWeight={600} fontFamily="Arial" textAnchor="middle">No concept data</text>
      </svg>
    );
  }
  ...
  return (
    <svg width="100%" height="100%" viewBox="0 0 1080 1920" preserveAspectRatio="xMidYMid meet">
      ...
      <rect width={width} height={1080} fill="url(#concept-bg)" />
      ...
      <text x={540} y={1280} fill="#94a3b8" fontSize={20} fontWeight={600} fontFamily="Arial" textAnchor="middle">{title}</text>
    </svg>
  );
};
